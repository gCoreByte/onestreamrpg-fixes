...
let autoSellList = [];
let forceSellList = localStorage.getItem('forceSellList') ? JSON.parse(localStorage.getItem('forceSellList')) : [];
...
document.querySelector("#craft").addEventListener("click", function () {
    ...
    let craftInventoryDivs = document.querySelectorAll(".crafting");
    let stashInventoryDivs = document.querySelectorAll("#stashInventory");
    let settingsDiv = document.querySelector("#settingsMenu");
...
function updateForcesellSelector() {
    let element = document.getElementById("settingsForcesell");
    // clear old options
    element.innerHTML = "";
    forceSellList.forEach(function (item) {
...
document.querySelector("#settingsForcesell").addEventListener("click", function () {
    event.stopPropagation();
    $('.context-menu-list').trigger('contextmenu:hide');
    updateForcesellSelector();
});

document.querySelector("#settingsForcesellRemove").addEventListener("click", function () {
    event.stopPropagation();
    $('.context-menu-list').trigger('contextmenu:hide');
    let element = document.getElementById("settingsForcesell");
    forceSellList = forceSellList.filter(item => item !== element.value);
    updateForcesellSelector();
});

document.querySelector("#settingsMenu").addEventListener("click", function () {
    event.stopPropagation();
    $('.context-menu-list').trigger('contextmenu:hide');
});
...
function loadInventory(user, force = false) {
    ...
    for (let i = 0; i < inv.length; i++) {
        ...
        if (inv[i].name != "empty") {
            ...
            // Force autosell any item mentioned here
            if (forceSellList.includes(inv[i].name) && inv[i].lock === true) {
                lock(inventoryItem.invPosition, false);
                // Don't bother selling it - autosell will handle it for us next time inventory gets updated
            }
        }
        else {
            inventoryItem = {};
...
function lock(slotNumber, isStash) {
    // event might not exist if we call it from loadInventory
    event?.stopPropagation();
    jwt = window.Twitch.ext.viewer.sessionToken;
    fetch(myServer + '/lock', {
        method: 'POST',
...
function toggleForcesell(itemName) {
    event.stopPropagation();
    updateForcesellSelector();
    if (!forceSellList.includes(itemName)) {
        console.log('Adding ' + itemName + ' to forceSellList');
        forceSellList.push(itemName);
    } else {
        console.log('Removing ' + itemName + ' from forceSellList');
        forceSellList = forceSellList.filter(item => item !== itemName);
    }
    localStorage.setItem('forceSellList', JSON.stringify(forceSellList));
    console.log('forceSellList:', forceSellList);
}
